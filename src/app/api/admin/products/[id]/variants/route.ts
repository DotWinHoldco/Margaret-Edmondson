...
export async function GET(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data, error } = await auth.supabase
    ...
    .select(
      'id, product_id, name, medium, size_label, width_in, height_in, is_custom_size, size_tier, lumaprints_cost_cents, shipping_cost_cents, margin_override_pct, manual_price_override_cents, is_active, is_lumaprints_available, last_priced_at, excluded_subcategory_ids',
    )
    .eq('product_id', id)
    .not('medium', 'is', null)
