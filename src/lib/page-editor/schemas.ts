...
const FEATURED_GRID_CONFIG_FIELDS: FieldSchema[] = [
  ...
  { kind: 'boolean', key: 'show_prices', label: 'Show prices on cards' },
  {
    ...
    itemLabel: (item) => (item.title as string) || (item.slug as string) || 'Product',
    addLabel: 'Add product',
    emptyLabel: 'No products yet. Add one and paste its slug.',
    itemFields: [
      { kind: 'plain-text', key: 'slug', label: 'Product slug', description: 'Paste from /shop/art/<slug>.' },
      { kind: 'plain-text', key: 'title', label: 'Display title (optional override)' },
      { kind: 'image', key: 'image_url', label: 'Override image (optional)', defaultCategory: 'products' },
    ],
  },
]
