...
> Source: official OpenAPI docs at https://api-docs.lumaprints.com (open beta), captured **2026-06-15**. This is the authoritative contract for the ArtByME integration. Field types, required/optional flags, and enums below are taken verbatim from each endpoint's OpenAPI spec.
...
- `200` correctly sized → `{ message, imageUrl, recommendedWidth, recommendedHeight (px), expectedAspectRatio (e.g. "2:3"), actualImageWidth, actualImageHeight, actualImageAspectRatio }`.
...
**Recommendation:** treat **whole inches as the safe default** for the size grid, and verify fractional acceptance in sandbox (the Phase 0 probe in `claude-code-prompt__custom-sizing-lumaprints.md` does exactly this). Because the **1% aspect rule** must hold regardless, the robust approach is: pick the print `width×height` that best matches the master's aspect (whole inches if required), then **pad the master to that exact print aspect (white border)** so `checkImageConfig`/submit always passes. This is the "white border on custom-size prints" the inventory already anticipates.
...
> The three ❌ rows are why orders can't be fulfilled today; the `submitOrder` row is also where custom `width`/`height` must be added. Fixes are specified in `claude-code-prompt__custom-sizing-lumaprints.md` (Phase 4). The `getProductsCost` ✅ row is the live custom-size price source the build wires into variant pricing (Phase 2).
...
*Endpoint specs captured from https://api-docs.lumaprints.com `.md`/OpenAPI on 2026-06-15. Re-verify against the dashboard before go-live (API is in open beta).*
