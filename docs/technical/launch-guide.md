...
The [September 11 input audit](launch-guide-input-audit-2026-09-11.md) maps every guide/editor control to its saved output and downstream behavior. Contact, shipping and product drafts now block accidental navigation/closure until saved or explicitly discarded. Product saves report their pending state to the guide; prices cannot be acknowledged while unsaved. The final store-opening and live-payment actions include the reviewed settings revision. `test/launch-wiring.test.tsx` exercises actual UI requests through production handlers with an isolated adapter, followed by the actual storefront/checkout functions. Real PostgreSQL transaction checks run separately with `node scripts/test-studio-db.mjs`.
