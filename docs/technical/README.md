...
- Admin access: a validated session and admin/artist role, enforced in the `(admin)` server
  layout for pages and in `requireAdmin` for API routes. See `docs/technical/admin-access.md`.
- Domains: storefront/checkout, fulfillment, LMS/account, CRM/email, page builder/funnels,
  social calendar, media library.

## Where the detail lives

- Admin access and sign-in requirements: `docs/technical/admin-access.md`.
