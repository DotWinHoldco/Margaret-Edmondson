...
4. Password updates require the validated recovery session and matching account. Authenticator-app verification was removed at the client’s request on September 18, 2026; see `admin-access.md`.
...
- Focused tests cover the request helper, normal/recovery callback routing, redirect protection, invalid/used links, missing/expired/changed sessions, mismatched passwords, policy errors/retry, password updates without factor APIs, and gated-route boundaries.
- Full `npm run build-check`: GREEN. Report: `docs/build-checks/password-recovery-build-check.txt`.
- Isolated Chrome fixtures: desktop and 390px mobile form rendering, expired-link recovery, and reset-request confirmation. No browser errors or horizontal overflow. Fixtures replace Supabase at its client boundary and cannot send email or change accounts.
