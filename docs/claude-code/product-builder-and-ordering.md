...
You are rebuilding the **print variant system, the admin product builder, the storefront product detail page, and the LumaPrints ordering pipeline** so Margaret can sell her originals at **true-to-aspect custom print sizes**. The schema migrations are already written (`supabase/migrations/2026061601_variant_custom_sizing.sql`, `2026061602_retire_legacy_print_variants.sql`). Build the code that uses them.

> **This is the authoritative build.** The exact LumaPrints API contract — endpoints, payloads, and the broken-endpoint delta you must fix — is in **`docs/lumaprints-api-reference.md`**; read it before writing any LumaPrints code.
...
- Admin builder: `src/components/admin/VariantsTab.tsx` (incl. inline `AddVariantModal`), `src/app/(admin)/admin/products/[id]/edit/page.tsx` (passes `artworkOrientation` from master px).
...
4.2 **Custom variant creator** (in `VariantsTab.tsx`, replacing the catalog-only `AddVariantModal`) — full UI in **Appendix C**:
...
4. Decide where the crop worker runs (a small always-on worker/container or an operator-run `scripts/` job) — see Appendix A.
